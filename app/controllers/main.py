...
class MainController:
    ...
    def view_extended_ticket(self):
        ...
        try:
            tickets, has_next = self.service.get_tickets(dto_request)
            ticket = self.service.get_ticket(id=id)
            data  = ticket.__dict__ 
            return render_template('main/index.html', tickets = tickets, current_page = page, has_next= has_next, ticket=data)
        except Exception:
            flash("Произошла ошибка", "danger")
            return render_template('main/index.html', tickets=[], current_page = page)
